...
class RentingResearch:
    ...
    def get_houses_info(self):
        ...
        response = requests.get(RENTING_WEBSITE_LINK, headers=headers)
        soup = BeautifulSoup(response.content, "html.parser")

        houses_ul = soup.find(id="grid-search-results").find(name="ul")
        lst = houses_ul.find_all(name="a", attrs={"class": "property-card-link", "tabindex": "0"}, recursive=True)

        for l in lst:
            link = l["href"]  # Link to the renting house
